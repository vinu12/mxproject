...
<tbody>
<?php 
if(!empty($detailsadvertise))
{
	$i=1;
foreach($detailsadvertise as $record)
{
...
<td><?php echo $record['page_title']; ?></td>
			<td><?php 
				...
				$result = $CI->Register_model->allcat($record['priority']);
...
<a class="btn btn-sm btn-info" href="<?php echo base_url().'admin_adminusers/editadvertise/'.base64_encode($record['id']); ?>" title="Edit"><i class="fa fa-pencil"></i></a>
<a class="btn btn-sm btn-danger deleteUser" href="javascript:void(0);" onclick="deleteadvertise('<?php echo base64_encode($record['id']);?>'); " title="Delete"><i class="fa fa-trash"></i></a>
</td>
</tr>
...
<script type="text/javascript">
    ...
    function checkstatus(userid,status)
    {
        ...
        if (confirm("Are you sure to " + msg)) {
            ...
            url = "<?php echo base_url(); ?>admin_adminusers/updateadvertisestatus";
            $.ajax({
                type: "POST",
                url: url,
	...
	function deleteadvertise(del)
	{
		 var msg = 'delete this deals';
		 
		  if (confirm("Are you sure to " + msg)) {
           
            url = "<?php echo base_url(); ?>admin_adminusers/deleteadvertise";
            $.ajax({
                type: "POST",
                url: url,
                data: {id: del},
