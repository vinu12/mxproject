...
public function joblist()
{
    ...
    $final_output = array();
			...
			if($create_at==0)
			{
				$create_at1= "WHERE jobtitle='International Guru' AND status='1' ORDER BY `id` DESC LIMIT 1) UNION (SELECT * FROM job WHERE jobtitle!='International Guru' AND status='1' ORDER BY `id` DESC LIMIT 10)";
			}else
			{
				 $create_at1= "WHERE jobtitle!='International Guru' AND status='1' AND create_at < $create_at ORDER BY `id` DESC LIMIT 10)";
			...
			$query = $this->db->query("(SELECT * FROM `job` ".$create_at1." ");
			$check_records = $query->result();
		    $count = $query->num_rows();
